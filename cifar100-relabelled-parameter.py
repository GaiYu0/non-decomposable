...
parser.add_argument('--batch-size-c', type=int, default=100)
...
parser.add_argument('--n-iterations-critic', type=int, default=10)
...
excluded = ('ckpt_every', 'gpu', 'log_every', 'n_iterations', 'resume')
experiment_id = 'cifar100-parameter-' + '-'.join('%s-%s' % (key, str(getattr(args, key))) for key in keys if key not in excluded)
