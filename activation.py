...
args.lra = 1e-4 # learning rate of actor
...
experiment_id = 'activation#' + '#'.join('%s:%s' % (key, str(getattr(args, key))) for key in keys if key not in excluded)
if args.tensorboard:
...
def forward(actor, batch_list, z=None, yz=True, L=True):
...
for i in range(args.resume, args.resume + args.ni):
    ...
    for j in range(args.nia):
        _, yz, L = forward(actor, batch_list)
        if args.tensorboard:
