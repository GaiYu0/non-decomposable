...
args.n_iterations_critic = 50
args.iw = 'sqrt' # ''/linear/quadratic/sqrt
args.gpu = 0
args.n_iterations = 5000
args.n_perturbations = 500
...
parser.add_argument('--n-iterations-critic', type=int, default=50)
parser.add_argument('--iw', type=str, default='sqrt')
parser.add_argument('--gpu', type=int, default=0)
parser.add_argument('--n-iterations', type=int, default=5000)
parser.add_argument('--n-perturbations', type=int, default=500)
...
run_id = '-'.join('%s-%s' % (key, str(getattr(args, key))) for key in keys)
...
test_loader = utils.data.DataLoader(utils.data.TensorDataset(test_x, test_y), 4096, drop_last=False)
...
def sqrt(x):
    x = th.abs(x)
    return th.sqrt(x) / th.max(x) ** 0.5

iw = {
    '' : lambda x: th.zeros(x.shape),
    'linear' : lambda x: x,
    'quadratic' : lambda x: x * x / th.max(x),
    'sqrt' : sqrt,
}
...
critic = my.RN(args.batch_size_c, 2 * n_classes, tuple(), (4 * n_classes, 64, 256), (256, 64) + (1,), F.relu, triu=True)
...
#     hist[-1]['c_optim_state_dict'] = my.optim_state_dict_gpu2cpu(c_optim.state_dict())
    ...
    for j in range(args.n_perturbations):
        ...
        L_bar_listz.append(L_mini_batch(y, y_bar_listz[-1]))
    ...
    # TODO batchify
    z_bar_list = [forward(y, y_bar).detach() for y_bar in y_bar_list]
    z_bar_tensor = th.cat(z_bar_list, 0)
    
    chunk = lambda x: th.chunk(x, int(args.topk / args.batch_size_critic), 0)
