...
parser.add_argument('--batch-size-c', type=int, default=200)
...
parser.add_argument('--n-iterations-critic', type=int, default=5)
...
experiment_id = 'cifar100-99-1-parameter-' + '-'.join('%s-%s' % (key, str(getattr(args, key))) for key in keys if key not in excluded)
