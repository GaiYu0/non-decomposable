...
run_id = 'subset' + '-'.join('%s-%s' % (key, str(getattr(args, key))) for key in keys)
...
train_x, train_y, _, _ = data.load_cifar10(rbg=True, torch=True)
# train_x, train_y, _, _ = data.load_cifar10(rbg=False, torch=True)

train_x, train_y = train_x[:args.dataset_size], train_y[:args.dataset_size]

train_set = utils.data.TensorDataset(train_x, train_y)
train_loader = utils.data.DataLoader(train_set, args.dataset_size, drop_last=False)
...
for key, value in global_scores(c, train_loader).items():
    print(key, value)
...
#             mse = th.sum(w * delta * (th.sign(delta) - th.sign(t)))    
    ...
    print('[iteration %s%d]' % (prefix, i + 1) +           ' | '.join('%s %0.3f' % (key, value) for key, value in train_scores.items()))
